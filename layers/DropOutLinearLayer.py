...
class DropOutLinearLayer:
    def __init__(self, 
        ...
        self.drop_out_mask = None  # Mask for dropout, initialized to None
    ...
    def forward(self, x: np.ndarray, training: bool = True):
        ...
        if training:
            if self.drop_out_rate > 0.0:
                # Create a dropout mask with the same shape as a_l
                self.drop_out_mask = (np.random.rand(*a_l.shape) > self.drop_out_rate).astype(np.float32)
                # Apply the dropout mask to a_l
                a_l = a_l * self.drop_out_mask
                a_l = 1/(1-self.drop_out_rate) * a_l
    ...
    def backpropagate(self, grad_next_layer, weights_next_layer):
        ...
        if self.drop_out_rate:
            grad_l = grad_l * self.drop_out_mask
            grad_l = 1/(1-self.drop_out_rate) * grad_l
